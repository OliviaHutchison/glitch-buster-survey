...
function GameOverMenu(reason){
    ...
    if (G.highscore < (G.currentLevel - 1)){
        G.highscore = G.currentLevel - 1
    }
    if (reason === 2) {
    ...
    ss.push(/*nomangle*/'high score: '/*/nomangle*/ + G.highscore);
    ...
    var s4 = ss[3],
        t4 = 8, // slightly smaller text if you want
        w4 = requiredCells(s4) * t4;

    this.button(cache(w4, t4 * 5 + 5, function(r){
    drawText(r, s4, 0, 5, t4, '#444');
    drawText(r, s4, 0, 0, t4, '#fff');
    }), (CANVAS_WIDTH - w4) / 2, 340);
...
function MainMenu(){
    ...
    this.button(button(/*nomangle*/'tutorial'/*/nomangle*/), 0, 420, G.tutorial);
...
function Game(){
    ...
    G.highscore = 0; // "Highscore: ", G.highscore
    ...
    G.controlsMenu = function(){
        G.menu = new ControlsMenu();
    };
