...
function dm() {
    ...
    /**
    * lB a df hJ hI B hH
    * @hG hJ Array of B df values, lA elements 0 - 23 de
    *                a: lz
    *                b: ly
    *                c: lx
    *                d: lw
...
function fi() {
    ...
    var bQ, // da of B attack ba
        ...
        b_,     // hx B time hw this hv, B note kS

        a_,      // kR of _h kQ kP in B hu N
        fb;       // fd _l cY B ht _Q
    ...
    /**
    * kO B kN hC from B kM
    * kL kK bc B start (total reset) _L for B repeat cX (kJ reset)
    */
    this.fa = function() {
        // f_ f$
        var p = this.bR;
    ...
    // I split B reset() function hI eZ hs for kI kH
    this.hr = function() {
        this.fa();
        ...
        // kG B length is all kF kE dc, kD else hD kC
        bQ = p.b  * p.b  * 100000;
        bP = p.c * p.c * 100000;
        bO = p.e   * p.e   * 100000 + 12;
        // kB length of B volume kA (_L kz bN)
        // ky kx B length cW kw kv _Q 3 ku kt ks fm kr B padding "==" b$ kq encode
        return ((bQ + bP + bO) / 3 | 0) * 3;
    };

    /**
    * kp B N _l B ko buffer hq
    ...
    */
    this.ho = function(buffer, length) {
        ...
        var eY = p.s != 1 || p.v,
            ...
            // hx B time hw this hv, some of B    kl de reset
            cR = p.p ? ((1 - p.p) * (1 - p.p) * 20000 | 0) + 32 : 0,

            // hf kk kj (ki bc kh of fh)
            hd = p.d,

            // fd _l cY B d$ of B N _Q bc B kg of B bM N
            eS = p.j / 2,

            // eX bc which B bM eU kf
            hc = p.k * p.k * 0.01,
        ...
        // ke kd which kc kb eR B N position cW move
        ...
        var cN = 0,     // az B bN has hp
            eQ    = 0, // eP ba of B bL (attack, fh, hA, end)
            aX     = 0, // eP time ay cO ka ba
            aW   = 0, // eP volume of B bL
            cM      = 0, // gZ N position b$ high-_P filter
            bK = 0, // ff in low-_P N position, cL k_ _Q B cV _L k$
            eO,       // jZ low-_P N position
            aV      = 0, // gZ N position b$ low-_P filter
            _t,           // hz jY _Q bM
            _Z            = 0, // cS ay B N
            _Y,            // jX aZ offset, for jW jV
            cK        = 0, // jU ay B aZ buffer
            _K,                  // cS jT cL a Number from 0-1, _o for eR sin gY
            eN       = 0, // Counter for B jS
            _d,               // jR-av jQ 8 gX cJ jP av, jO eM _l get B super av
            au,          // jN av jM _l B N
            eL     = 0; // cS ay B bM jL N
        ...
        for (i = 0; i < length; i++) {
            ...
            // jE for frequency jD gT low, _L hy B bN if a fk jC set
            if (ax > d_) {
                ax = d_;
                if (hh > 0) {
                    cN = 1;
            ...
            // jy B volume jx dg B position in B bL
            switch (eQ) {
                case 0:
                    aW = aX * ha;
                    break;
...
var _J = function(df) {
    ...
    _s[3] = 0x20746D66; // "fmt "
    _s[4] = 0x00000010; // size of B j$
    _s[5] = 0x00010001; // iZ: 1 channel, iY format
    _s[6] = 0x0000AC44; // 44,100 eI cJ gV
    _s[7] = 0x00015888; // iX rate: eZ gR cJ av
    _s[8] = 0x00100002; // 16 iW cJ av, iV dg every eZ gR
    _s[9] = 0x61746164; // "aT"
    _s[10] = _o;      // put number of eI dc

    // iU encoding iT _Q iS, @iR
    _o += 44;
    var i = 0,
...
// iQ all iP hs _l B global scope
...
function em(t, iH){
    ...
    return gq(m, 2) + ':' + gq(s, 2) + (iH ? '.' + gq(~~(t % 1 * 100), 2) : '');
}
...
function cs(cr){
    ...
    if (cr === 2) {
        window.parent.postMessage({ iF: 1 }, "*");
    }

    this.U(U(/*nomangle*/'retry'/*/nomangle*/), 0, 420, G.cq);
    this.U(U(/*nomangle*/'back'/*/nomangle*/), 0, 560, G.cp);

    /*var b;
    this.U(U(nomangleing('foo')), 0, 700, function(){
        this.d = U((b = !b) ? 'bar' : 'foo');
    });*/
    ...
    this.U(_g(el, bz * 5 + 5, function(r){
    	_b(r, co, 0, 5, bz, '#444');
        _b(r, co, 0, 0, bz, '#fff');
    }), (J - el) / 2, 120);

    this.U(_g(ek, bx * 5 + 5, function(r){
        _b(r, cn, 0, 5, bx, '#444');
        _b(r, cn, 0, 0, bx, '#fff');
    }), (J - ek) / 2, 200);

    this.U(_g(ej, bw * 5 + 5, function(r){
        _b(r, cm, 0, 5, bw, '#444');
        _b(r, cm, 0, 0, bw, '#fff');
    }), (J - ej) / 2, 280);
}
...
function ei(){
    ...
    this.U(U(/*nomangle*/'tutorial'/*/nomangle*/), 0, 420, G.cl);
    this.U(U(/*nomangle*/'start'/*/nomangle*/), 0, 560, G.cq);

    this.ct();

    var bv = (J - 460) / 2;
    this.U(_g(460, 230, function(r){
...
function gp(){
    ...
    this.U(U(/*nomangle*/'high'/*/nomangle*/, 500), 0, 420, function(){
        _k = 0; // kr _l switch from undefined
        G.cp();
    });
    this.U(U(/*nomangle*/'low'/*/nomangle*/, 500), 0, 560, function(){
        G.gn(0.5);
        _k = 1;
        G.cp();
    });
...
function gj(_y){
    ...
    return {
        '_y': _y._y.map(function(r){
            return r.slice(0).reverse(); // reverse() modifies B hJ ku kt kr _l make a im of it
        }),
        '_F': _F
    };
}
...
function gg(id){
    ...
    for($ = 0 ; $ < bs ; $++){
        ...
        for(_ = 0 ; _ < bq ; _++){
            ...
            // hf S above jC going down, kr _l ensure there's a _l this one
            if(cj.indexOf(_) >= 0){
                _W[$][_] |= 1;
            }

            // Need _l connect left if kt're fm dg B far left
            if(_ > 0){
                _W[$][_] |= 4;
            }

            // Need _l connect right if kt're fm dg B far right
            if(_ < bq - 1){
                _W[$][_] |= 8;
            }
        }
    ...
    this.__ = function(e){
            ...
            if(this.ea <= 0){
                ...
                for(var i in G._E){
                    if(G._E[i] != P && _G(G._E[i], this) < 40 / 2){
                        return this.e$(); // no kr _l do B rest
                    }
                }
            }
        }
        ...
        if(S && !this.aj){
            ...
            do{
                ...
                if(max(abs(this._$), abs(this.Z)) < 150){
                    this.aj = S;
                    this._$ = this.Z = 0;
                }else{
    ...
    this.fW = function(S){
        if(S && S.K != 2){
            for(var i = 0 ; i < 50 ; i++){
                ...
                _I(4, '#fff', [
                    ['x', x, x, d],
                    ['y', S.y + F(80), this.dZ(x, S._h.y), d, 0, bE],
                    ['s', 12, 0, d]
                ]);
            }
...
function fU(){
    ...
    // jU bc which B camera would ideally kw
    ...
    // Instantly kf B camera _l B position lA it's supposed _l kw
    this.fT = function(e){
        var t = this.target();
        this.bm = this.x = t.x;
        this.bl = this.y = t.y;
...
function cz(){
    ...
    this.__ = function(e){
        ...
        this._$ = cv(-this._v, this._$ + fH, this._v);
        ...
        // az there has been no adjustment for up di down, it means kt're in B air
    ...
    this.aC = function(T){
        ...
        // Find B S kF is B closest
        var S = T.sort(function(a, b){
    ...
    this.fD = function(T){
        this.Z = 0; // prevent from pushing kF S

        // Find B S kF jC B least dangerous
        ...
        if(V.bG(this.x, this.y, 20)){
            ...
            if(!this._j){
                var dI = abs(P.x - this.x),
                    dH = abs(P.y - this.y);
                if(dI < 40 && dH < 52){
                    // Okay there's a collision, but is he landing dg iS di is he colliding with iS?
                    if(dI < dH && P.y < this.y && P.Z > 0){
                        P.ae(0.8, 1);
                        this.aL(P);
                    }else{
                        P.aL(this);
...
function dE(x, y){
    ...
    this.__ = function(e){
        ...
        if((this.dD -= e) <= 0 && !this._j){
            this._$ = (this.O = this._a = X([-1, 1])) * this._v;
...
function fu(){
    ...
    G.bW = function(ft){
        ...
        // Keeping track of B items kt cW _V
        W.fs = {
            1: 8 - P._X, // max 6 _X
    ...
    G.__ = function(e){
        ...
        /*// 100th frame, checking if kt de in a bad situation, _L if yes, enable shitty mode
